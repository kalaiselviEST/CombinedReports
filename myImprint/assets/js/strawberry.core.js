...
var Strawberry = function () {
  ...
  this.Loader = function (_parent) {
    ...
    _this.showFull = function () {
      ...
      $("body").append(
        '<div class="spinner-backdrop"><div class="spinner-wrap"><div class="spinner"></div></div></div>'
      );
  ...
  this.Workspace = function (_parent) {
    ...
    _this.load = function (ele) {
      ...
      setTimeout(function () {
        $("#workspace").css({ transform: "translateY(-10%)", opacity: "0" });
      }, 100);
      ...
      setTimeout(function () {
        $("#workspace").load(url + ".html", function (response, status, xhr) {
          ...
          if (typeof window.workspaceScript === "object") {
            try {
              ...
              console.groupCollapsed(
                "Strawberry Core Error: Workspace Script onLoad not defined"
              );
              console.warn(err);
              console.groupEnd();
            }
          ...
          if ($('[data-workspace-src="' + url + '"]').length > 0) {
            ...
            $("title").text(
              $('[data-workspace-src="' + url + '"]').data("title")
            );
            $(".header-title").text(
              $('[data-workspace-src="' + url + '"]').data("page-header")
            );
          } else {
    ...
    _this.initRef = function () {
      //This function is only for design and NOT needed in implementation
      var initHash = window.location.hash || window.defaultUrl;
  ...
  this.Sidebar = function (_parent) {
    ...
    this.toggler = function () {
      ...
      $(".menu-toggle").click(function (e) {
        ...
        if($('.pagination-footer-fixed').hasClass('expand')){          
          $('.pagination-footer-fixed').removeClass('expand');
        }else{
          $('.pagination-footer-fixed').addClass('expand');
        }
    ...
    this.submenuToggler = function () {
      ...
      $(".submenu-wrapper [data-toggle=submenu]").click(function () {
        ...
        _menuparent.find(".submenu-wrapper.open > .submenu").animate(
          ...
          function () {
            _menuparent
              .find(".submenu-wrapper.open > .submenu")
              .removeClass("show");
    ...
    this.workspace.toggler = function () {
      ...
      //   _parent.dropdownSelectListImageToggler();
      ...
      $(document).on("click", '[data-toggle="workspace-sidebar"]', function () {
        _parent.dropdownSelectToggler();
        _parent.dropdownSelectListImageToggler();
        $($(this).data("target")).addClass("show");
        $($(this).data("target"))
          .find("[data-dismiss=sidebar]")
          .on("click", function () {
            $(this)
    ...
    _this.confirm = function (options) {
      ...
      options = {
        ...
        yes:
          options.yes ||
          function () {
            console.log("Strawberry Confirm Dialog: Clicked Yes");
          },
        no:
          options.no ||
          function () {
            console.log("Strawberry Confirm Dialog: Clicked No");
          },
      ...
      _modal
        .find(".modal-header i")
        .attr("class", "mr-1 text-primary fas fa-" + options.icon);
      _modal.find(".modal-body").addClass("py-4");
      _modal.find(".modal-dialog").addClass("modal-sm");

      var _btnY = $(
        "<button class='btn btn-sm btn-outline-success border-0' data-dismiss='modal'><i class='fas fa-check mr-1'></i>Yes</button>"
      );
      var _btnN = $(
        "<button class='btn btn-sm btn-outline-secondary border-0' data-dismiss='modal'><i class='fas fa-times mr-1'></i>No</button>"
      );

      if (typeof options.yes == "function") _btnY.click(options.yes);
      if (typeof options.no == "function") _btnN.click(options.no);
      ...
      _modal.on("shown.bs.modal", function (e) {
        ...
        _modal
          .find(".modal-content")
          .draggable({ handle: ".modal-header", containment: "body" });
      });
    };
  };
  ...
  this.tooltipToggler = function () {
    ...
    $(
      '[data-toggle="tooltip"]:not([data-original-title]):not(.no-tooltip)'
    ).tooltip({
      boundary: "viewport"
    });
  ...
  this.fieldsetToggler = function () {
    ...
    $('[data-toggle="fieldset-disable"]').click(function () {
      ...
      $(
        '[data-toggle=fieldset][data-target="' + $(this).data("target") + '"]'
      ).show();
  ...
  this.fieldsetSwitch = function () {
    $('[data-fieldset-toggle]').change(function () {
      $($(this).data('fieldset-toggle')).prop('disabled', $(this).prop('checked')).toggleClass('disabled');


      if ($(this).prop('checked')) {
        $($(this).data('fieldset-toggle-inverted')).addClass('disabled');
      } else {
        $($(this).data('fieldset-toggle-inverted')).removeClass('disabled');
      }

    })

    $('[data-fieldset-toggle-inverted]').change(function () {
      $($(this).data('fieldset-toggle-inverted')).prop('disabled', !$(this).prop('checked'));
  ...
  this.dropdownSelectListImageToggler = function () {
    //For using dropdowns as delct control

    $(".dropdown:not(.dropdown-select):visible [data-select-list-img]").each(
      function () {
        $(this)
          .closest(".dropdown")
          .find(".dropdown-item")
          .click(function () {
            ...
            _ele
              .closest(".dropdown")
              .find(".text")
              .append(_ele.find("img").clone());

            var _title = _ele
              .find(".list-img-title")
              .parent()
              .clone();
            _title.find(":not(.list-img-title)").remove();
  ...
  this.dropdownFormBubbler = function () {
    ...
    $(
      ".dropdown.dropdown-form:visible > .dropdown-menu:not(.dropdown-no-bubble)"
    ).each(function () {
      _ele = $(this);
  ...
  this.Toast = function (_parent) {
    ...
    _this.show = function (_body, _type, _callback) {
      ...
      switch (_type) {
        ...
        case "warning":
          _icon =
            "<i class='fas fa-exclamation-circle fa-fw text-warning mr-2'></i>";
          _title = "Warning";
          break;
        case "danger":
          _icon =
            "<i class='fas fa-exclamation-triangle fa-fw text-danger mr-2'></i>";
          _title = "Alert";
          break;
      }
      ...
      var _html =
        '<div id="' +
        _id +
        '" class="toast show animated fadeInRight shadow-lg border-' +
        _type +
        '" role="alert" aria-live="assertive" aria-atomic="true"><div class="toast-header">';
      _html += _icon;
      _html +=
        '<strong class="mr-auto text-' + _type + '">' + _title + "</strong>";
      ...
      switch (_type) {
        case "success":
          _icon =
            "<div class='toast-icon'><i class='fas fa-check-circle fa-fw'></i></div>";
          break;
        case "info":
          _icon =
            "<div class='toast-icon'><i class='fas fa-info-circle fa-fw'></i></div>";
          break;
        case "primary":
          _icon =
            "<div class='toast-icon'><i class='fas fa-info-circle fa-fw'></i></div>";
        ...
        case "danger":
          _icon =
            "<div class='toast-icon'><i class='fas fa-exclamation-triangle fa-fw'></i></div>";
          break;
      }
